...
// ✅ Cálculo de número de semana (ISO 8601) - versión segura
...
// ✅ parseWeekInput corregido (sin UTC, más compatible)
function parseWeekInput(weekStr) {
  ...
  const firstMonday = new Date(year, 0, 1 + (1 - offset + 7) % 7);
...
function initResidenteModal() {
  ...
  // ✅ Asignamos onclick directamente (más seguro)
...
function loadMealsGrid(user) {
  ...
  // ✅ Semana actual (sin UTC, más compatible)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setupApp() {
    ...
    const btnCocinero = document.getElementById("btn-cocinero");

    if (btnResidente) {
      btnResidente.onclick = initResidenteModal;
    }

    if (btnCocinero) {
      btnCocinero.onclick = initCocineroModal;
    }
