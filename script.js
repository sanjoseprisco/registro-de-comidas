...
// ✅ Cálculo ISO 8601: Número de semana
function getWeekNumber(date) {
  ...
  const days = Math.floor((d - jan4) / 86400000);
  return Math.ceil((days + jan4.getDay() + 1) / 7);
}

// ✅ parseWeekInput: Calcula el lunes de la semana W36
...
function loadMealsGrid(user) {
  ...
  const defaultWeek = `${year}-W${String(weekNum).padStart(2, '0')}`;
  ...
  const render = () => {
    ...
    // ✅ Depuración: Verifica que weekDate es correcto
    console.log("Semana seleccionada:", weekInput.value, "Lunes:", weekDate.toDateString());
