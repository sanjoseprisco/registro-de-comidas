    ...
    dates.forEach(date => {
      ...
      ["breakfast", "lunch", "dinner"].forEach(meal => {
        ...
        input.addEventListener("change", () => {
          ...
          updateMealsSummary(user, dates, meals); // Actualizar resumen
        });
  ...
  if (container) {
    container.innerHTML = `
      <div class="meals-summary">
        ...
        <strong>${breakfasts} desayunos</strong>,
        <strong>${lunches} comidas</strong>,
        <strong>${dinners} cenas</strong>.
      </div>
    `;
  }
}

function setupReports(user) {
  ...
  weeklyBtn.addEventListener("click", () => {
    ...
    dates.forEach(date => {
      const dateStr = formatDate(date);
      const meals = stored[dateStr] || {};
      const des = meals.breakfast ? "🍳" : "–";
      const com = meals.lunch ? "🍲" : "–";
      const cen = meals.dinner ? "🥗" : "–";
      html += `<li><strong>${formatDay(date)}</strong>: ${des} ${com} ${cen}</li>`;
    });
    html += "</ul>";
  ...
  monthlyBtn.addEventListener("click", () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (stored[dateStr]?.breakfast) summary[key].breakfast++;
      if (stored[dateStr]?.lunch) summary[key].lunch++;
      if (stored[dateStr]?.dinner) summary[key].dinner++;
    }
