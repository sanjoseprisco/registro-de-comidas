...
function setupSaveButton() {
  ...
  saveButton.addEventListener("click", () => {
    ...
    Array.from(grid.querySelectorAll(".meal-day")).forEach(dayBox => {
      const dateStr = dayBox.querySelector("input").dataset.date;
      const dayOptions = dayBox.querySelectorAll("input[type='checkbox']");

      dayOptions.forEach(input => {
        const meal = input.dataset.meal;
        if (input.checked) {
          if (!meals[dateStr]) meals[dateStr] = {};
          meals[dateStr][meal] = true;
        }
      });
    });
